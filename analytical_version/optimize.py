    ...
    for prob in data:
        ...
        for use_coarse_to_fine in [True, False]:
            ...
            for i in range(1):
                ...
                nu_lowlevel_vel = torch.zeros(
                    (batch_size, horizon - 1, problem.num_agents)
                )
                transition_by = 1000
                update_alm_every = 10
                ...
                update_alm_penalty_terms_until = (
                    update_alm_after
                    + update_alm_penalty_term_iterations * update_alm_every
                )
                ...
                for step in tqdm.tqdm(range(total_steps)):
                    ...
                    curves["agent_obstacle_penalties"].append(
                        obstacle_penalties.tolist()
                    )
                ...
                plt.subplot(2, 3, 3)
                plt.plot(
                    curves["lowlevel_vel_penalties"], label="lowlevel_vel_penalties"
                )
                plt.title("V")
                plt.yscale("log")
                ...
                plt.tight_layout()
                plt.savefig(
                    base_dir + f"/optimization_curve_{use_coarse_to_fine}_{i}.png"
                )
                ...
                with open(
                    base_dir + f"/solution_{use_coarse_to_fine}_{i}.pkl", "wb"
                ) as f:
                    pickle.dump(
                        {
                            "agent_positions": sol.agent_positions.detach()
                            .cpu()
                            .numpy(),
                ...
                for step in range(problem.num_timesteps):
                    ...
                    problem.visualize(
                        SolutionValue(
                            ...
                            agent_positions=sol.agent_positions[0, step : step + 1]
                            .detach()
                            .cpu()
                            .numpy(),
                        ),
                ...
                with av.open(
                    base_dir + f"/trajectory_{use_coarse_to_fine}_{i}.mp4", "w"
                ) as container:
        ...
        # plt.legend()
        # plt.tight_layout()
        # plt.savefig(base_dir + "/summary_histogram.png")
